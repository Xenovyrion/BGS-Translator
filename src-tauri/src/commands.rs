...
pub async fn open_plugin_cmd(
    ...
    tracing::info!("[cmd] open_plugin: {} (target_lang={})", path, target_lang);
    ...
    let loaded = open_file(std::path::Path::new(&path), &lang, move |status| {
        ...
        tracing::error!("[cmd] open_plugin failed: {}", e);
        e.to_string()
    })?;

    let entry_count = loaded.entries.len();
    tracing::debug!("[cmd] open_plugin: streaming {} entries in chunks of 200", entry_count);
...
#[tauri::command]
#[tracing::instrument(skip(session, app))]
pub async fn save_session_cmd(
    session: TranslationSession,
    app: tauri::AppHandle,
) -> Result<String, String> {
    tracing::info!("[cmd] save_session: {} ({} entries)", session.plugin_name, session.entries.len());
    let id = save_session_auto(&session, &app).map_err(|e| {
        tracing::error!("[cmd] save_session failed: {}", e);
        e
    })?;
    tracing::debug!("[cmd] session saved as '{}'", id);
    Ok(id)
}

#[tauri::command]
pub async fn list_sessions_cmd(app: tauri::AppHandle) -> Result<Vec<SessionListItem>, String> {
    tracing::debug!("[cmd] list_sessions");
...
pub async fn load_session_cmd(
    ...
    tracing::info!("[cmd] load_session: {}", id);
    let result = load_session_by_id(&id, &app);
    match &result {
        Ok(s)  => tracing::info!("[cmd] session loaded: {} ({} entries)", s.plugin_name, s.entries.len()),
        Err(e) => tracing::error!("[cmd] load_session failed: {}", e),
    }
    result
}
...
pub async fn delete_session_cmd(
    ...
    tracing::info!("[cmd] delete_session: {}", id);
...
pub async fn import_translations_from_plugin_cmd(
    ...
    tracing::info!("[cmd] import_translations_from: {}", reference_path);
    ...
    let loaded = open_file(std::path::Path::new(&reference_path), "en", |_| {}).map_err(|e| {
        tracing::error!("[cmd] import_translations_from failed: {}", e);
        e.to_string()
    })?;
    tracing::info!("[cmd] import_translations_from: {} entries loaded from reference", loaded.entries.len());
    Ok(loaded.entries)
}
...
pub async fn open_strings_file_cmd(
    ...
    tracing::info!("[cmd] open_strings_file: {}", path);
    ...
    let table = load_strings_file(p).map_err(|e| {
        tracing::error!("[cmd] open_strings_file failed: {e}");
        e.to_string()
    })?;
    ...
    tracing::info!("[cmd] open_strings_file: {} entries, stem={}", entry_count, plugin_stem);
...
pub async fn ensure_dir_cmd(path: String) -> Result<(), String> {
    tracing::info!("[cmd] ensure_dir: {}", path);
    std::fs::create_dir_all(&path).map_err(|e| {
        let msg = format!("Cannot create directory '{}': {}", path, e);
        tracing::error!("[cmd] {}", msg);
        msg
    })
}
...
pub async fn export_plugin_cmd(
    ...
    tracing::info!("[cmd] export_plugin: {} ({} / {} validated entries)", output_path, validated, total);
    ...
    if result.is_ok() {
        tracing::info!("[cmd] export_plugin completed successfully");
    } else {
        tracing::error!("[cmd] export_plugin failed: {:?}", result);
    }
    result
}
...
pub async fn export_strings_cmd(
    ...
    let validated = entries.iter().filter(|e| e.status == crate::translation::entry::EntryStatus::Validated).count();
    tracing::info!("[cmd] export_strings: plugin={} lang={} pack_ba2={} validated={}/{}",
    ...
    let result = if pack_as_ba2 {
        ...
        tracing::info!("[cmd] export_strings: writing BA2 → {}", output_path.display());
        crate::parser::strings_writer::write_strings_ba2(
            ...
            tracing::error!("[cmd] export_strings (BA2) failed: {}", e);
            e
        })?
    } else {
        // ── Loose files output ────────────────────────────────────────────────
        crate::parser::strings_writer::write_strings_files(
            ...
            tracing::error!("[cmd] export_strings (loose) failed: {}", e);
            e
        })?
    };

    let total = result.strings + result.dlstrings + result.ilstrings;
    tracing::info!("[cmd] export_strings done — strings={} dlstrings={} ilstrings={} total={} ba2={}",
...
pub async fn import_format_cmd(path: String) -> Result<Vec<formats::ImportedEntry>, String> {
    tracing::info!("[cmd] import_format: {}", path);
    let p = std::path::Path::new(&path);
    let ext = p.extension().and_then(|e| e.to_str()).unwrap_or("").to_lowercase();
    ...
    match &result {
        Ok(entries) => tracing::info!("[cmd] import_format: {} entries loaded", entries.len()),
        Err(e)      => tracing::error!("[cmd] import_format failed: {}", e),
    }
    result
}
...
pub async fn export_xtranslator_xml_cmd(
    ...
    tracing::info!("[cmd] export_xtranslator_xml: {} ({} entries)", path, entries.len());
    ...
    tracing::info!("[cmd] export_xtranslator_xml: {} entries written", count);
    Ok(count)
}
...
pub async fn export_esptranslator_xml_cmd(
    ...
    tracing::info!("[cmd] export_esptranslator_xml: {} ({} entries)", path, entries.len());
    let count = formats::esptranslator_xml::export(
        std::path::Path::new(&path),
        &entries,
        &plugin_name,
    )?;
    tracing::info!("[cmd] export_esptranslator_xml: {} entries written", count);
    Ok(count)
}
...
pub async fn export_session_csv_cmd(
    ...
    tracing::info!("[cmd] export_session_csv: {} ({} entries)", path, entries.len());
    let count = formats::session_csv::export(std::path::Path::new(&path), &entries)?;
    tracing::info!("[cmd] export_session_csv: {} entries written", count);
    Ok(count)
}
    ...
    for entry in entries.filter_map(|e| e.ok()) {
        ...
        if modified < cutoff {
            if std::fs::remove_file(&path).is_ok() {
                removed += 1;
                tracing::debug!("[logs] Purged old log: {}", path.display());
            }
        }
    }
